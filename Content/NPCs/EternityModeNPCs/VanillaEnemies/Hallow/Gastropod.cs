    ...
    public class Gastropod : EModeNPCBehaviour
    {
        ...
        public override void OnKill(NPC npc)
        {
            ...
            if (FargoSoulsUtil.HostCheck && npc.HasPlayerTarget)
            {
                ...
                for (int i = 0; i < 12; i++)
                    Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, vel.RotatedBy(2 * Math.PI / 12 * i), ProjectileID.PinkLaser, FargoSoulsUtil.ScaledProjectileDamage(npc.defDamage, 0.8f), 0f, Main.myPlayer);
            }
        }
    }
}
