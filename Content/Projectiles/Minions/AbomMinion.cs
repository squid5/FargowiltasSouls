                ...
                if (npc.CanBeChasedBy(Projectile))
                {
                    ...
                    if (++Projectile.localAI[0] > 15)
                    {
                        ...
                        if (Projectile.owner == Main.myPlayer)
                            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity + Projectile.SafeDirectionTo(npc.Center) * 30, ModContent.ProjectileType<AbomMinionScythe>(), Projectile.damage, Projectile.knockBack / 2, Projectile.owner, npc.whoAmI);
                    }
                }
                else //forget target
                {
