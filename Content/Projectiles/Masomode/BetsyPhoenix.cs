...
namespace FargowiltasSouls.Content.Projectiles.Masomode
{
    public class BetsyPhoenix : ModProjectile
    {
                ...
                for (int i = 0; i < 2; ++i)
                {
                    if (!Main.rand.NextBool(4))
                    {
                        Dust dust = Dust.NewDustDirect(Projectile.Center - Projectile.Size / 4f, Projectile.width / 2, Projectile.height / 2,
                            recolor ? dustID : Utils.SelectRandom(Main.rand, new int[3] { 6, 31, 31 }), 0.0f, 0.0f, 0, default, 1f);
                        dust.noGravity = true;
                        dust.velocity *= 2.3f;
                        dust.fadeIn = 1.5f;
                        dust.noLight = true;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            int dustID = WorldSavingSystem.EternityMode && SoulConfig.Instance.BossRecolors ? DustID.Shadowflame : DustID.Torch;
            for (int i = 0; i < 10; i++)
            {
                int dust = Dust.NewDust(Projectile.position, Projectile.width,
                    Projectile.height, dustID, 0f, 0f, 100, default, 3f);
                Main.dust[dust].noGravity = true;
                Main.dust[dust].velocity *= 7f;
                dust = Dust.NewDust(Projectile.position, Projectile.width,
                    Projectile.height, dustID, 0f, 0f, 100, default, 1f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (WorldSavingSystem.EternityMode && SoulConfig.Instance.BossRecolors)
                texture2D13 = ModContent.Request<Texture2D>("FargowiltasSouls/Content/Projectiles/Masomode/BetsyPhoenix_Recolor").Value;
