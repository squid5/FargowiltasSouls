...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 uv : TEXCOORD0) : COLOR0
{
    ...
    float colorMult = 1;
    if (border) 
        colorMult = InverseLerp(radius * 0.6, radius, worldDistance);
    else
    {
        colorMult = InverseLerp(radius * 1.05, radius, worldDistance);
    }
    ...
    // Orange color
    float4 darkColor = float4(0.8, 0.05, 0, 1);
    float4 midColor = float4(0.95, 0.275, 0.027, 1);
    float4 lightColor = float4(0.95, 0.9, 0.9, 1);
