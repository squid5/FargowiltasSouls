...
float4 Wings(float2 uv : TEXCOORD0) : COLOR0
{
	...
	float3 noise = tex2D(noiseImage, float2(frac(uv.x * .9 + globalTime * 0.3), uv.y * 0.5 + globalTime * 0.2)).rgb;
	...
	float3 noise2 = tex2D(colorImage, float2(frac(0.6 - globalTime * 0.5), globalTime * 0.2 + length(uv) * 0.5)).rgb + (result * 0.3);
